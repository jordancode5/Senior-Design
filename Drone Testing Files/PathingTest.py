...
def main() :
    ...
    drone = DroneCtrlNoEyes.Drone(fly_speed, fly_height, fall_speed, f, sim_bool)
    ...
    while(True):
        if(State == 'A'):
            ...
            f.write("Going to path list: %s\n" %path_index)
            f.write("Current location: %s" % drone.get_location())
