...
# what ArUco dictionary am I detecting?
...
# initialize video capture to webcam detected
cap = cv.VideoCapture(0)
cap.set(cv.CAP_PROP_FRAME_WIDTH, 1280)  # 1280 pixels wide
cap.set(cv.CAP_PROP_FRAME_HEIGHT, 720)  # 720 pixels tall

# load my YOLOv8 Model
path = "C:\\Users\\Jordan\\Downloads"       # file location PC
# path = "C:\\Users\\jorda\\Downloads"    # file location laptop
model_architecture = 'best4'            # file name
model = YOLO(path + f'/{model_architecture}.pt')    # define YOLO model

while True:
    # start time to compute the fps
    start = datetime.datetime.now()
    ret, frame = cap.read()     # read camera capture information in frame

    if not ret:
        break

    # run the YOLO model on the frame
    detections = model(frame)[0]                    # how do these detections play into the system?
    ...
    # cv.imshow("YOLOv8 Tracking", annotated_frame)  # will display the tracked frame

    # this line creates an output stream
    annotated_frame = pose_estimation(annotated_frame, ARUCO_DICT[aruco_type], intrinsic_camera, distortion)
    ...
    cv.putText(annotated_frame, fps, (50, 50),
                cv.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 255), 8)

    cv.imshow("YoloV8 Tracking", annotated_frame)       # will display frame

# pipe.stop()       # used in realsense scenario
cap.release()       # stop video stream
cv.destroyAllWindows()  # close windows created
