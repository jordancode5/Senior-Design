...
class Drone:
    ...
    captured_ids = []
    # initialize drone
    def __init__(self, flyspeed, flyheight, fallspeed, f, simulated=False):
        # connect to drone
    ...
    def track_hover(self, id, duration):
        ...
        while (time.perf_counter() < time_end):
            ...
            if (coords == 0):  # no target in view after 5 seconds
                return 0
            coords = coords[1]          #first line of input
    ...
    def attack_ugv(self, id):
        ...
        """
        # there is a bug in the code below dealing with id array formatting
        # you get the pleasure of finding it
        # good luck!!!
        print("ID =", id)       # debug statement. Useful for looking at what your formatting is 
        coords = self.wait_scan(id)
        if (coords == 0):  # no target in view after 5 seconds
            return 0
        coords = coords[1]  # coords = (x,y,z)
        while (coords[2] > self.hover_height):  # will fly down until set height above the ground, then fire the water system
        ...
        if (not self.simulated):
            self.ACE.fire_laser(fire_len)       # this will be FireSystem instead of Ace
