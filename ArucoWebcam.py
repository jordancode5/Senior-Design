...
def scan_ugv(self, id):         # need to tie this to some drone object
    # this is essentially going to be your pose estimation that returns tvec for a single ID
    # scan for IDs that have not been eliminated
    # id = self.id there needs to be some global variable associated with this input
    gray = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)
    cv.aruco_dict = cv.aruco.getPredefinedDictionary("DICT_6X6_250")        #dict I will be using
    parameters = cv.aruco.DetectorParameters()
    corners, ids, rejected_img_points = cv.aruco.detectMarkers(gray, cv.aruco_dict, parameters=parameters)
    # you are going to get the tvec
    if len(corners) > 0:
        for i in range(0, len(ids)):
            #print("pose estimation loop has run", i, "times")
            #print("current id =", ids[i])
            rvec, tvec, markerPoints = cv.aruco.estimatePoseSingleMarkers(corners[i], 0.096, matrix_coefficients,
                                                                          distortion_coefficients)
            if np.isin(ids[i], targetList) == False:
                #self.id = ids[i]
